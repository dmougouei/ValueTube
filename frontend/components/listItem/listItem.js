...
module.exports = function ListItem(props) {
    try {
        if (typeof props === "object" || props instanceof Object) {
            ...
            let minutes = Math.floor((this.props.metadata.duration - (hours * 3600)) / 60);
            ...
            if (this.props.metadata.view_count < 1000) {
                ...
                views = (this.props.metadata.view_count/1000000000000).toFixed(1) + "T";
            }
            ...
            return `
                <div class="list-item${this.props.small ? " sm" : ""}" onclick="window.location.href = './watch?v=${this.props.metadata.id}'">
                    <div class="preview">
                        <img alt="${this.props.metadata.title}" src="${this.props.metadata.thumbnail}" loading="lazy" decoding="async" />
                        <div class="duration">${(hours != 0) ? `${hours}:${minutes}:${seconds}` : `${minutes}:${seconds}`}</div>
                    </div>
                    <div class="details">
                        <div class="title">${
                            (this.props.small && this.props.metadata.title.length > 58)
                                ? `${this.props.metadata.title.substring(0, 58).substring(0, this.props.metadata.title.lastIndexOf(" "))} ...`
                                : this.props.metadata.title
                        }</div>
                        <div class="content-creator youtube-link" data-url="${this.props.metadata.uploader_url}">${this.props.metadata.uploader}</div>
                        ...
                        ${Container({
                            ...
                            content: this.props.small ? `` : (
                                this.props.metadata.description.length > 270
                                    ? `${this.props.metadata.description.substring(0, 270).substring(0, this.props.metadata.description.lastIndexOf(" "))} ...`
