This the source code for the pytorch implementation of the deepmoji machine learning model, with additions. Several scripts have been written for implementing the deepmoji model in this project, described below:

vt_score_emoji_individual.py:
Tkenizes comments, then applies the model and generates emojis and percentiles for each comments. Adds these to a table in an SQL database.

insert_emojid_db.py:
Inserts values from a csv file output from vt_score_emoji_individual.py into an SQL databse (e.g. if csv is used in =stead of database, and these need to be added to a database later).

kmeans_emojized.py:
Selects from table to get all emoji results for the comment on each video, apply kmeans clustering for dimensionality reduction. Testing is still in progress and will be testing other methods.

SQLite is being used for testing currently, but this will change before production.

All scripts writted so far are in the /examples directory and can be run from the terminal like so:

python3 vt_score_emoji_individual.py
