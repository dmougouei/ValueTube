...
def fit_model(model, loss_op, optim_op, train_gen, val_gen, epochs,
    ...
    for epoch in range(epochs):
        for i, data in enumerate(train_gen):
            n_train, m_train = data
            X_train = Variable(n_train, requires_grad=False)
            y_train = Variable(m_train, requires_grad=False)
            model.train()
            optim_op.zero_grad()
        ...
        val_loss = np.mean(
            [calc_loss(loss_op, model(Variable(xv)), Variable(yv)).data.cpu().numpy()[0] for xv, yv in val_gen])
